...
# Colecciones públicas oficiales observadas en la navegación de Tost. El
# endpoint JSON de Shopify evita cargar imágenes, fuentes y JavaScript, por lo
# que este collector no necesita Playwright.
    ...
    try:
        for index, section in enumerate(CATALOG_SECTIONS, start=1):
            ...
            try:
                for page_number in range(1, MAX_PAGES_PER_SECTION + 1):
                    endpoint = (
                        f"{BASE_URL}/collections/{section.handle}/products.json"
                        f"?limit={PAGE_SIZE}&page={page_number}"
                    )
                    with metrics.measure("http"):
                        response = session.get(endpoint, timeout=REQUEST_TIMEOUT)
                    page_products: dict[str, CollectedProduct]
                    page_cards: int
                    if response.status_code == 200 and "json" in response.headers.get("content-type", "").casefold():
                        with metrics.measure("parse"):
                            page_products, page_cards = _parse_shopify_payload(response.json(), section.name)
                    else:
                        # Respaldo HTML si Shopify restringe products.json.
                        html_url = f"{BASE_URL}/collections/{section.handle}?page={page_number}&view=boost-pfs-original"
                        with metrics.measure("http_fallback"):
                            html_response = session.get(html_url, timeout=REQUEST_TIMEOUT)
                        html_response.raise_for_status()
                        with metrics.measure("parse"):
                            page_products, page_cards = _parse_html(html_response.text, section.name)
                    ...
                    print(
                        f"Tost {section.key} página {page_number}: HTTP={response.status_code}, "
                    ...
                    # Shopify devuelve una lista vacía al terminar. En fallback
                    # HTML se corta al no aparecer productos nuevos.
                    if page_cards == 0 or not page_products or new_page == 0:
                        break
                    if page_cards < PAGE_SIZE and response.status_code == 200 and "json" in response.headers.get("content-type", "").casefold():
                        break
        ...
        return CollectionBatch(
            ...
            stats=CollectionStats(
                ...
                discovery_source="configured-shopify-collections",
